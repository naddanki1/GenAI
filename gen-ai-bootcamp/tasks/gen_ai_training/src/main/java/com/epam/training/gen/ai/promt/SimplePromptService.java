...
public class SimplePromptService {
    ...
    public List<String> getChatCompletions(String prompt, String model) {
        String relevantKnowledge = knowledgeStoreService.getRelevantKnowledge(prompt);

        // If no relevant knowledge is found, let the AI know
        if (relevantKnowledge == null) {
            relevantKnowledge = "No relevant knowledge found in the knowledge base.";
        }

        // Step 2: Combine the retrieved knowledge with the user's query to form a full prompt
        String fullPrompt = relevantKnowledge + "\nUser Query: " + prompt;
        ...
        if(imageModels.containsKey(model)){
            ...
            return generateTextResponse(fullPrompt, deploymentOrModelName);
        }
    }
